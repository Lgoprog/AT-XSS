...
from agent_core import Agent2, Agent3
...
class AbstractDQNAgent(Agent2):
    ...
    def compute_batch_q_values(self, state_batch):
        ...
        ##when models > 2
        # q_values = self.model.predict_on_batch([batch,batch])
...
class DQNAgent(AbstractDQNAgent):
    ...
    def compile(self, optimizer, metrics=[]):
        ...
        print('训练网络：',trainable_model.summary())
    ...
    def backward(self, reward, terminal,observation_1):
        ...
        if self.step > self.nb_steps_warmup and self.step % self.train_interval == 0:
            ...
            if self.enable_double_dqn:
                ...
                # when models > 2
                # q_values = self.model.predict_on_batch([state1_batch,state1_batch])
                ...
                # when models > 2
                # target_q_values = self.target_model.predict_on_batch([state1_batch,state1_batch])
            ...
            ##因为这里多个输入用的list因此ins被判断为了list类型
            ##when model > 2
            # ins = [state0_batch,state0_batch] if type(self.model.input) is list else state0_batch
