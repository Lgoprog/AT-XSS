...
from Rainbow.utils import *
from IPython.display import clear_output
from torch.nn.utils import clip_grad_norm_
from Rainbow.preprocess_frame import *
import copy
from Rainbow.frame_stack import *
import os
...
class DQNAgent:
    ...
    def select_action(self, state: np.ndarray) -> np.ndarray:
        ...
        q_value = self.dqn(torch.FloatTensor(state).unsqueeze(0).to(self.device)).max().detach().cpu()
    ...
    def train(self, num_frames: int) -> (List[int], List[int]):
        ...
        q_values = []  # q-score for each episode
        frame_scores = []  # average score each frame_interval frames
        frame_q_values = [] #average q_value each frame_interval frames
        ...
        for frame_idx in range(1, num_frames + 1):
            ...
            if frame_idx % self.frame_interval == 0:  ##每五轮取平均值损失函数
                if len(scores) == 0:
                    ...
                    frame_q_values.append(float(np.mean(q_values)))
                if self.plot:
                ...
                # print('print models:',self.dqn)
                # summary(self.dqn,(1,257))
        ...
        return frame_scores, losses,frame_q_values
