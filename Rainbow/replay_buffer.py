...
import numpy as np
import random
from Rainbow.segment_tree import *
...
class ReplayBuffer:
    ...
    def __init__(
        ...
        self.next_obs_buf = np.zeros([int(size), *obs_dim], dtype=np.float32)
        self.acts_buf = np.zeros([int(size)], dtype=np.float32)
        self.rews_buf = np.zeros([int(size)], dtype=np.float32)
        self.done_buf = np.zeros(int(size), dtype=np.float32)
        self.max_size, self.batch_size = int(size), batch_size
