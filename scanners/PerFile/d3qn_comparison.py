...
class W13SCAN(PluginBase):
    ...
    def D3QN_Per(self,param=None, type=None):
        ...
        window_length = 1  ##表示模型记忆时只记忆单条样本
        ep_reward_list = []

        # hyper parameters
        num_frames = 5000   ##总的步长
        memory_size = 5    ##memory中长度
        batch_size = 5         ##跟memory一样才可以loss
        target_update = 20     ##这是硬更新平频率相当于5*5，每5个样本更新一次模型
        plotting_interval = 1   ##每个向量一次
        plot = False
        # train
        ...
        # agent_ddqn = DQNAgent(env, memory_size, batch_size, target_update,
        #                  no_dueling=True, no_categorical=True, no_double=False,
        #                  no_n_step=True, no_noise=True, no_priority=True,
        #                  plot=plot, frame_interval=plotting_interval)
        # agent_dueldqn = DQNAgent(env, memory_size, batch_size, target_update,
        #                  no_dueling=False, no_categorical=True, no_double=True,
        #                  no_n_step=True, no_noise=True, no_priority=True,
        #                  plot=plot, frame_interval=plotting_interval)
        # agent_d3qn = DQNAgent(env, memory_size, batch_size, target_update,
        #                  no_dueling=False, no_categorical=True, no_double=False,
        #                  no_n_step=True, no_noise=True, no_priority=True,
        #                  plot=plot, frame_interval=plotting_interval)

        agent_d3qn_per = DQNAgent(env, memory_size, batch_size, target_update,
                         no_dueling=False, no_categorical=True, no_double=False,
                         no_n_step=True, no_noise=True, no_priority=False,
                         plot=plot, frame_interval=plotting_interval)
        #agents = [agent_dqn,agent_ddqn,agent_dueldqn,agent_d3qn,agent_d3qn_per]
        agents = [agent_d3qn_per]
        labels = ["DQN", "Double DQN", "Dueling DQN","D3QN","D3QN-PER"]
        scores = []
        losses = []
        q_values = []
        ...
        plt.savefig('d3qn_scores.png',dpi=600)
        plt.close()
        #plt.subplot(122)
        #Q-value
        plt.title('Training episode: %s' % num_frames)
        ...
        plt.savefig('d3qn_q_values.png',dpi=600)
        plt.close()
        #loss:
        plt.title('Training episode: %s' % num_frames)
        ...
        plt.savefig('d3qn_q_losses.png',dpi=600)
        plt.close()
    def audit(self):
